        ...
        for(size_t i = 0; i<display_getSize();){
            ...
            for(moduleProperty_t p = no_property+1; p < end_of_properties;p++){
                ...
                if( requestedProperties & (1<<p) && http_modulePropertyHandlers[p].toJson){
                    ...
                    cJSON_AddItemToObject(json,propertyNames[p],property);
                }
            }

            size_t buf_offset = strlen(buf);
            if(!cJSON_PrintPreallocated(json,buf+buf_offset,MAX_HTTP_BODY_SIZE-buf_offset-10,true) || i == display_getSize()-1){
                strcat(buf,",");
                // could not print --> so buffer is filled and should be flushed
                if(i == display_getSize()-1){
                    // all modules are added to the buffer
                    buf[strlen(buf)-1]=']'; // replace last ',' with ']'
                    i++;
                }
...
esp_err_t api_set_http_modulePropertyHandlers(httpd_req_t *req)
{
    ...
    int recv_cnt = 0;
    // Receive data. Only a json array is starting with '[' at index 0 is accepted. A large json can be read an parsed in multiple sections.
    while((recv_cnt = httpd_req_recv(req, buf+to_parse_size, MAX_HTTP_BODY_SIZE-1-to_parse_size))){
        ...
        if(json_first_object_start == 0 || json_first_object_start > json_last_object_end || json_last_object_end == MAX_HTTP_BODY_SIZE){
            httpd_resp_set_status(req, "422 Unprocessable Entity");
            httpd_resp_send(req, NULL, 0);
            return ESP_OK;
        }
        ...
        cJSON_ArrayForEach(json_module, json){
            ...
            free(string);
            for(moduleProperty_t p = no_property+1; p < end_of_properties;p++){
                cJSON* property = cJSON_GetObjectItemCaseSensitive(json_module, propertyNames[p]);
...
static const httpd_uri_t api_get_module_endpoint = {
    .uri          = "/api/modules",
    .method       = HTTP_GET,
    .handler      = api_get_http_modulePropertyHandlers,
    .user_ctx     = NULL
};
...
static const httpd_uri_t api_set_module_endpoint = {
    .uri          = "/api/modules",
    .method       = HTTP_POST,
    .handler      = api_set_http_modulePropertyHandlers,
    .user_ctx     = NULL
};
...
static const httpd_uri_t wifi_ap_post = {
    .uri          = "/api/wifi",
    .method       = HTTP_POST,
    .handler      = wifiCredentials_post_handler,
    .user_ctx     = NULL
};
