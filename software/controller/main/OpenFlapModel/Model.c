        ...
        if(ulTaskNotifyTake(true, 250 / portTICK_RATE_MS) == fromHttp){
            ...
            for(moduleProperty_t property = no_property+1; property < end_of_properties; property++){
                if(ctx.controller->display.requestedProperties & (1<<property)){
                    ...
                    ESP_LOGI(TAG, "read property: %d",property);
            ...
            for(moduleProperty_t property = no_property+1; property < end_of_properties; property++){
                updatablePropertiesWriteAll |= ((updatableProperties & (1<<property) && uart_moduleSerializedPropertiesAreEqual(property)) << property);
            }
            ...
            ESP_LOGI(TAG, "Updatable properties with WriteSequential command: 0x%04llX",updatablePropertiesWriteSequential);
            for(moduleProperty_t property = no_property+1; property < end_of_properties; property++){
...
bool display_getPowered(){
    return gpio_get_level(FLAP_ENABLE);
}
void display_setPowered(bool powered){
    gpio_set_level(FLAP_ENABLE, powered);
