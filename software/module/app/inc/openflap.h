...
typedef struct of_ctx_tag {
    ...
    uint32_t encoder_sensor_calibration_ticks; /**< The number of ticks remaining for encoder calibration. */
    ...
    struct {
        ...
        uint16_t normalized[ENCODER_CHANNEL_COUNT]; /**< Normalized ADC values for each encoder channel. */
        uint16_t min[ENCODER_CHANNEL_COUNT]; /**< Minimum ADC values for each encoder channel, used for calibration. */
        uint16_t max[ENCODER_CHANNEL_COUNT]; /**< Maximum ADC values for each encoder channel, used for calibration. */
...
/**
 * \brief Start the encoder sensor calibration process.
 *
 * \param[inout] ctx A pointer to the openflap context.
 */
...
/**
 * \brief Run the encoder sensor calibration loop.
 *
 * \param[inout] ctx A pointer to the openflap context.
 */
...
/**
 * \brief Check if the encoder sensor calibration process is ongoing.
 *
 * \param[inout] ctx A pointer to the openflap context.
 * \return True if the calibration process is ongoing, false otherwise.
 */
