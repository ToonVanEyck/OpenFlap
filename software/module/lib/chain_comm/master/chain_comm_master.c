...
cc_master_err_t cc_property_read_all(cc_master_ctx_t *ctx, cc_prop_id_t property_id)
{
    ...
    CC_RETURN_ON_FALSE(cc_master_uart_read(ctx, &rx_header, sizeof(rx_header), NULL) == sizeof(rx_header),
                       CC_MASTER_ERR_FAIL, TAG, "Failed to receive header");
    ...
    CC_RETURN_ON_FALSE(cc_master_uart_read(ctx, &module_cnt, sizeof(module_cnt), NULL) == sizeof(module_cnt),
                       CC_MASTER_ERR_FAIL, TAG, "Failed to receive module count");

    /* Update the node count. */
    ctx->master.node_cnt_update(ctx->master_userdata, module_cnt);
    ...
    for (uint16_t i = 0; i < module_cnt; i++) {
        ...
        CC_RETURN_ON_FALSE(cc_master_uart_read(ctx, property_data, property_size, &rx_checksum_calc) == property_size,
                           CC_MASTER_ERR_FAIL, TAG, "Failed to receive read all data");
...
cc_master_err_t cc_property_write_all(cc_master_ctx_t *ctx, cc_prop_id_t property_id)
{
    ...
    /* Send the header. */
    CC_RETURN_ON_FALSE(cc_master_uart_write(ctx, &tx_header, sizeof(tx_header), &tx_checksum_calc) == sizeof(tx_header),
                       CC_MASTER_ERR_FAIL, TAG, "Failed to send header");
    ...
    if (CTX_PROP.attribute.write_size.is_dynamic) {
        ...
        CC_RETURN_ON_FALSE(cc_master_uart_read(ctx, &rx_property_size, sizeof(rx_property_size), NULL) ==
                           ...
                           CC_MASTER_ERR_FAIL, TAG, "Failed to receive property size header");
    ...
    ctx->uart.read_timeout_set(ctx->uart_userdata, RX_BYTES_TIMEOUT(property_size));
    CC_RETURN_ON_FALSE(cc_master_uart_read(ctx, rx_buff, property_size, NULL) == property_size, CC_MASTER_ERR_FAIL, TAG,
    ...
    ctx->uart.read_timeout_set(ctx->uart_userdata, RX_BYTES_TIMEOUT(1));
    CC_RETURN_ON_FALSE(cc_master_uart_read(ctx, &msg_rc.raw, 1, NULL) == 1, CC_MASTER_ERR_FAIL, TAG,
...
cc_master_err_t cc_property_write_seq(cc_master_ctx_t *ctx, cc_prop_id_t property_id)
{
    ...
    while (ctx->master.node_exists_and_must_be_written(ctx->master_userdata, node_idx, property_id,
                                                       &node_must_be_written)) {
        uint8_t tx_checksum_calc                    = 0;
        uint8_t property_data[CC_PROPERTY_SIZE_MAX] = {0};
