    ...
    if (property_size->is_dynamic) {
        while (ctx->uart.cnt_writable(ctx->uart_userdata) && ctx->data_cnt < 2 &&
            ...
            if (ctx->header.action == property_writeAll || ctx->header.action == property_readAll ||
                ...
                cc_node_uart_write(ctx, &((uint8_t *)&ctx->property_size)[ctx->data_cnt], 1, cs);
            }
            ctx->data_cnt++;
        };
    } else {
...
{
    /* Send data. */
    uint8_t data_cnt = cc_node_uart_write(ctx, &ctx->property_data[ctx->data_cnt], ctx->property_size - ctx->data_cnt,
